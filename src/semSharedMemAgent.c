...
static void prepareIngredients ()
{
    ...
    if(semUp(semgid, sh->ingredient[ing1]) == -1){ //notifing watcher
	perror ("error on the up operation for semaphore access (AG)");
	exit (EXIT_FAILURE);
    }  

    if(semUp(semgid, sh->ingredient[ing2]) == -1){ //notifing watcher
	perror ("error on the up operation for semaphore access (AG)");
	exit (EXIT_FAILURE);
    }
    /* TODO: insert your code here */
...
static void closeFactory ()
{
	...
	if(semUp(semgid, sh->ingredient[index]) == -1){ //notifing watcher
            perror ("error on the up operation for semaphore access (AG)");
            exit (EXIT_FAILURE);
        }
    }
